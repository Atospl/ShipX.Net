...
namespace ShipX.Net.Core
{
    public class ShipXClient : BaseClient
    {
        ...
        public DownloadResult<Sticker> DownloadShipmentLabelV2(string shipmentId, LabelFormatEnum format = LabelFormatEnum.Pdf, LabelTypeEnum type = LabelTypeEnum.normal)
        {
            RestRequest request = createDownloadShipmentLabel(shipmentId, format, type);
            IRestResponse response = RestClient.Execute(request);
            return new DownloadResult<Sticker>(response);
        }
